...
class Ti_Withe_Label_Admin {
	...
	private function get_strings() {
		...
		$white_label_description = sprintf( __( 'This will remove the white label module from the dashboard. If you want to access while label settings in future, simply deactivate the %s plugin and activate it again.', 'hestia-pro' ), $this->product_details['product_name'] );
		if ( $this->product_details['type'] === 'theme' ) {
			$white_label_description = sprintf( __( 'This will remove the white label module from customizer. If you want to access while label settings in future, simply change the %s theme and activate it again.', 'hestia-pro' ), $this->product_details['product_name'] );
		}
