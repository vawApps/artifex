...
class Hestia_Header_Addon extends Hestia_Header {
	...
	public function cart_item( $responsive = false ) {
		...
		$cart .= '<li class="' . esc_attr( $class ) . '"><a href="' . esc_url( wc_get_cart_url() ) . '" title="' . esc_attr__( 'View cart', 'hestia-pro' ) . '" class="nav-cart-icon"><i class="fa fa-shopping-cart"></i>' . trim( ( is_object( WC()->cart ) && ( WC()->cart->get_cart_contents_count() > 0 ) ) ? '<span>' . WC()->cart->get_cart_contents_count() . '</span>' : '' ) . '</a>';
		if ( (bool) $responsive !== true ) {
