...
class Hestia_Admin {
	...
	public function welcome_notice_content() {
		...
		$ob_btn = sprintf(
			...
			esc_html__( 'Go to the theme settings', 'hestia-pro' )
		);
		$options_page_btn = sprintf(
			...
			sprintf( esc_html__( 'or try one of our ready to use Starter Sites', 'hestia-pro' ) )
		);
		...
		$content = sprintf(
			...
			sprintf(
				esc_html__( 'Congratulations!', 'hestia-pro' ),
				$name
			),
			sprintf(
				/* translators: %s - theme name */
			...
			$ob_btn,
			$options_page_btn,
			esc_html__( 'Return to your dashboard', 'hestia-pro' )
		);
